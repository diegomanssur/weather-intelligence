# Massive + Weather + Lakebase Databricks App Boilerplate
...
- Calls the **National Weather Service API** (`api.weather.gov`) - free and keyless, just needs a descriptive `NWS_USER_AGENT` - to fetch active alerts and forecasts per location
- Syncs Massive API data and NWS weather data into Lakebase in batches
- Exposes a small Flask API to trigger syncs and read synced records
- Turns synced text (news articles, weather alerts/forecasts) into `pgvector` embeddings via scheduled Databricks Workflow notebooks, for downstream RAG exercises

## Files

- `app.py` - Flask app: `/healthz`, `/records` (GET), `/sync` (POST), `/watchlist` (GET/POST/DELETE), `/news/sync` (POST), and registers `weather_client.py`'s Blueprint for `/weather/sync` (POST)
...
- `weather_client.py` - NWS (`api.weather.gov`) client: geocodes `"City, ST"` locations via the free Census geocoder, resolves each to a grid point (`/points/{lat},{lon}`), fetches active alerts + gridpoint forecasts, normalizes both into `weather_documents` rows, and exposes the `/weather/sync` Blueprint route
- `setup_secrets.py` - One-time script to create the secret scopes and store the Massive API key + Lakebase URL (weather needs no secret - see [Endpoints](#endpoints))
...
- `notebooks/ingest_weather_embeddings.py` - Companion ETL notebook for weather: reads unembedded rows from `weather_documents` (populated by `POST /weather/sync`, not by this notebook), chunks `narrative_text` with a sliding window, embeds with the same `all-MiniLM-L6-v2` model as the news pipeline, and upserts into `weather_embeddings` (pgvector) via plain `psycopg2` - no Spark
- `databricks.yml` + `resources/ingest_ticker_news_embeddings_job.yml` + `resources/ingest_weather_embeddings_job.yml` - Databricks Asset Bundle config that schedules both notebooks above as Workflows (see [Scheduling the embeddings notebooks](#scheduling-the-embeddings-notebooks-as-databricks-workflows))
...
> **Weather integration needs no account or API key.** `weather_client.py` calls `api.weather.gov` (free, public, no auth) and the US Census Bureau geocoder (also free, no key) to resolve `"City, ST"` locations to lat/lon. The only thing it needs is a descriptive `NWS_USER_AGENT` value (app name + contact email/URL) set as a plain env var in `app.yaml` - not a secret, since NWS asks for this to identify traffic, not to authenticate it. Skip straight to step 2 if you only care about the weather sync.
...
For deployment, `app.yaml` already pulls `LAKEBASE_URL` from the `database/lakebase-url` secret automatically, and already sets `NWS_USER_AGENT` to a placeholder — replace that placeholder with your real app name + contact email/URL before deploying (NWS throttles requests with generic/default User-Agents).
...
- `POST /weather/sync` with JSON body `{"locations": ["Chicago, IL", "Austin, TX"], "limit": 50}` - geocode each location, fetch active alerts + gridpoint forecast from NWS, normalize, and upsert into `weather_documents`; returns `{"synced": <count>}`

## Scheduling the embeddings notebooks as Databricks Workflows

Both embeddings notebooks follow the same shape - read source rows, chunk, embed, upsert into
a `pgvector` table - and are scheduled the same way. See each subsection below.

### Ticker news: `notebooks/ingest_ticker_news_embeddings.py`
...
### Weather: `notebooks/ingest_weather_embeddings.py`

`notebooks/ingest_weather_embeddings.py` is *not* self-contained the way the news notebook is -
it doesn't call the NWS API itself. It only reads whatever `POST /weather/sync` has already
written into `weather_documents`, chunks any `narrative_text` longer than `chunk_size` (sliding
window, `chunk_size`/`chunk_overlap` chars, default 800/100), embeds each chunk with
`sentence-transformers/all-MiniLM-L6-v2` (the same model as the news pipeline, so both embedding
tables stay queryable with the same `vector_cosine_ops` distance operator), and upserts into
`weather_embeddings`. Make sure something is actually calling `POST /weather/sync` on its own
schedule (a cron hitting the app, a separate lightweight job, etc.) - this notebook won't trigger
that for you.

#### Option A: Databricks Asset Bundle (CLI, version-controlled)

This repo includes `resources/ingest_weather_embeddings_job.yml` alongside the ticker-news one;
`databricks.yml`'s `include: resources/*.yml` picks it up automatically, no bundle changes needed.

1. Set the real workspace URL in `databricks.yml` (replace `<your-workspace-instance>`), if you
   haven't already for the ticker-news job.
2. Deploy: `databricks bundle deploy -t dev`
3. Test it once manually: `databricks bundle run ingest_weather_embeddings_job -t dev`
4. Once confirmed, flip `pause_status: PAUSED` to `pause_status: UNPAUSED` in
   `resources/ingest_weather_embeddings_job.yml` and redeploy. Note this job defaults to running
   every 15 minutes rather than daily - weather alerts are time-sensitive in a way ticker news
   isn't, so adjust `quartz_cron_expression` to match how often you actually call `/weather/sync`.

#### Option B: Workflows UI (no CLI required)

Same steps as the ticker-news notebook's Option B, with these differences:
- Notebook path: `notebooks/ingest_weather_embeddings.py`
- Parameters:
  ...
  - `chunk_overlap` = `100`
  - `batch_size` = `32`
- No Massive-style secret scope/key parameters - this notebook doesn't call any external API.
- Cluster: a single-node cluster is enough (`num_workers: 0` in the bundle version) - this
  notebook has no Spark distribution step, unlike the news notebook's title/description embedding
  UDF.
- Schedule: consider every 15 minutes rather than daily, for the same reason as Option A above.
...
this app, plus `weather_documents` and `weather_embeddings` if you want the weather sync/embeddings
history tracked too:
...
- `weather_client.py` only fetches active alerts + the standard gridpoint forecast (`/forecast`) -
  it does not fetch NWS forecast discussion (AFD) text products, since those are keyed by forecast
  office via a separate `/products` endpoint rather than by lat/lon. Add that as a follow-up if you
  need discussion text specifically.
- `weather_client.py`'s geocoder (US Census Bureau, for `"City, ST"` → lat/lon) only covers US
  addresses. Locations outside the US, or ambiguous/misspelled city names, will fail to geocode -
  pass `"lat,lon"` directly for those instead.
